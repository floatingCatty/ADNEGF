...
class NEGF(object):
    def __init__(self, hmt_ovp: dict, positions: torch.Tensor, d_trains: int, offset, left_boundary=None, right_boundary=None,
            espacing=0.01, poissonMethod="image charge", sgfMethod='Sancho-Rubio', dc=1.0, n_img=200, Emin=-25, etaLead=1e-5,
            etaDevice=0, maxIter=100, conv_err=1e-7, cutoff=True, SEpreCal=True, dtype=torch.complex128, device='cpu', **SCFOptions):

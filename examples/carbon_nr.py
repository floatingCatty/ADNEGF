...
def graphene_first_nearest_neighbour():
    ...
    t = 2.8
    set_tb_params(PARAMS_C_C={'pp_pi': t})

    # --------------------------- Hamiltonian -------------------------

    h = Hamiltonian(xyz=coords, nn_distance=1.5)
    h.initialize()
    h.set_periodic_bc(period)

    band_structure = torch.zeros((sum(num_points), h.h_matrix.shape[0]))
    ...
    ax.plot(np.sort(band_structure.detach().numpy()), 'k')
...
def graphene_third_nearest_neighbour_with_overlaps():
    ...
    h = Hamiltonian(xyz=coords, nn_distance=[1.5, 2.5, 3.1], comp_overlap=True)
    h.initialize()
    h.set_periodic_bc(period)
...
def graphene_nanoribbons_armchair():
    ...
    hl, hd, hr, _, _, _, _ = h.get_hamiltonians_block_tridiagonal(optimized=True)
    energy = np.linspace(-3, 3., 200)
    tr = np.zeros(energy.shape)
    dos = np.zeros(energy.shape)

    for j, ee in enumerate(energy):
        L, R = negf.surface_greens_function(ee, hl, hd, hr, iterate=True, damp=1e-5j)
        print(L.shape)

        # compute Green's functions using the recursive Green's function algorithm
        g_trans, grd, grl, gru, gr_left = negf.recursive_gf(ee,
                                                            hl,
                                                            hd,
                                                            hr,
                                                            left_se=L,
...
def graphene_nanotube():
    ...
    h = Hamiltonian(xyz=coords, nn_distance=1.7, comp_angular_dep=False)
    h.initialize()
    h.set_periodic_bc(period)
